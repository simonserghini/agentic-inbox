...
function getSnoozeOptions() {
	...
	if (evening > now) {
		options.push({ label: "Later today (8pm)", value: () => evening.toISOString() });
	}
	...
	options.push({ label: "Next week", value: () => nextWeek.toISOString() });

	return options;
}

export default function SnoozePicker({ open, onClose, onSnooze }: SnoozePickerProps) {
	...
	const handleCustom = useCallback(() => {
		...
		onClose();
	}, [customDate, customTime, onSnooze, onClose]);
	...
	return (
		<div
			ref={ref}
			className="absolute top-full right-0 z-50 mt-1 min-w-[180px] rounded-lg border border-kumo-line bg-kumo-elevated shadow-lg py-1 animate-fade-in"
		>
			...
			{getSnoozeOptions().map((opt) => (
				<button
					...
					onClick={() => { onSnooze(opt.value()); onClose(); }}
				>
					{opt.label}
				</button>
			))}
